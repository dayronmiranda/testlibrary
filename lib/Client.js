...
const { ChatFactory, ContactFactory } = require('./structures');
...
class Client extends EventEmitter {
    ...
    async inject() {
        ...
        await exposeFunctionIfAbsent(this.pupPage, 'onAppStateHasSyncedEvent', async () => {
            ...
            if (!injected) {
                console.log('🔥 DEBUG: Store/WWebJS not injected, performing full injection...');
                    ...
                    await this.pupPage.evaluate(() => {
                        ...
                        window.WWebJS.getMessageModel = (message) => {
                            try {
                                ...
                                if (window.Store.Validators && window.Store.Validators.findLinks) {
                                ...
                                console.warn('Error in getMessageModel:', error);
                                return message.serialize ? message.serialize() : message;
                            }
                        };
                        ...
                        window.WWebJS.getChatModel = async (chat, { isChannel = false } = {}) => {
                            try {
                                ...
                                if (isChannel && window.Store.ChatGetters) {
                                ...
                                console.warn('Error in getChatModel:', error);
                        ...
                        window.WWebJS.getContacts = () => {
                            try {
                                ...
                                console.warn('Error in getContacts:', error);
                                return [];
                            }
                        };
                        ...
                        window.WWebJS.getContactModel = contact => {
                            try {
                                ...
                                console.warn('Error in getContactModel:', error);
                                return contact.serialize ? contact.serialize() : contact;
                            }
                        };

                        console.log('✅ Manual WWebJS injection completed');
                    });
                }
            } else {
                console.log('🔥 DEBUG: Store/WWebJS already injected, setting up ClientInfo...');
                
                // Even if injected, we still need to set up ClientInfo
                this.info = new ClientInfo(this, await this.pupPage.evaluate(() => {
                    return { ...window.Store.Conn.serialize(), wid: window.Store.User.getMeUser() };
                }));
            }
            
            // ALWAYS attach event listeners regardless of injection status
    ...
    async attachEventListeners() {
        console.log('🔥 DEBUG: attachEventListeners() called - setting up event handlers');
        
        await exposeFunctionIfAbsent(this.pupPage, 'onAddMessageEvent', msg => {
            console.log('🔥 DEBUG: onAddMessageEvent callback triggered!', {
            ...
            if (msg.type === 'gp2') {
                console.log('🔥 DEBUG: Processing group notification message');
            ...
            console.log('🔥 DEBUG: Creating Message object from msg data');
            const message = new Message(this, msg);

            console.log('🔥 DEBUG: Emitting MESSAGE_CREATE event');
            /**
            ...
            if (msg.id.fromMe) {
                console.log('🔥 DEBUG: Message is from me, not emitting MESSAGE_RECEIVED');
                return;
            }

            console.log('🔥 DEBUG: Emitting MESSAGE_RECEIVED event');
            /**
        ...
        await this.pupPage.evaluate(() => {
            console.log('🔥 DEBUG: Setting up WhatsApp Store event listeners...');
            
            // Debug: Check if Store and required objects exist
            console.log('🔍 DEBUG: Store availability check:', {
                hasStore: typeof window.Store !== 'undefined',
                hasMsg: typeof window.Store?.Msg !== 'undefined',
                hasAppState: typeof window.Store?.AppState !== 'undefined',
                hasConn: typeof window.Store?.Conn !== 'undefined',
                hasCall: typeof window.Store?.Call !== 'undefined',
                hasChat: typeof window.Store?.Chat !== 'undefined',
                hasWWebJS: typeof window.WWebJS !== 'undefined'
            });
            
            // Debug: Check if event handler functions exist
            console.log('🔍 DEBUG: Event handler functions check:', {
                hasOnChangeMessageEvent: typeof window.onChangeMessageEvent === 'function',
                hasOnChangeMessageTypeEvent: typeof window.onChangeMessageTypeEvent === 'function',
                hasOnMessageAckEvent: typeof window.onMessageAckEvent === 'function',
                hasOnAddMessageEvent: typeof window.onAddMessageEvent === 'function',
                hasOnRemoveMessageEvent: typeof window.onRemoveMessageEvent === 'function'
            });
            
            // Set up event listeners with debugging
            window.Store.Msg.on('change', (msg) => { 
                console.log('🔥 DEBUG: Store.Msg change event triggered for message:', msg.id?._serialized);
                window.onChangeMessageEvent(window.WWebJS.getMessageModel(msg)); 
            });
            
            window.Store.Msg.on('change:type', (msg) => { 
                console.log('🔥 DEBUG: Store.Msg change:type event triggered for message:', msg.id?._serialized, 'type:', msg.type);
                window.onChangeMessageTypeEvent(window.WWebJS.getMessageModel(msg)); 
            });
            
            window.Store.Msg.on('change:ack', (msg, ack) => { 
                console.log('🔥 DEBUG: Store.Msg change:ack event triggered for message:', msg.id?._serialized, 'ack:', ack);
                window.onMessageAckEvent(window.WWebJS.getMessageModel(msg), ack); 
            });
            
            window.Store.Msg.on('change:isUnsentMedia', (msg, unsent) => { 
                console.log('🔥 DEBUG: Store.Msg change:isUnsentMedia event triggered for message:', msg.id?._serialized, 'unsent:', unsent);
            ...
            window.Store.Msg.on('remove', (msg) => { 
                console.log('🔥 DEBUG: Store.Msg remove event triggered for message:', msg.id?._serialized, 'isNewMsg:', msg.isNewMsg);
            ...
            window.Store.Msg.on('change:body change:caption', (msg, newBody, prevBody) => { 
                console.log('🔥 DEBUG: Store.Msg change:body/caption event triggered for message:', msg.id?._serialized);
            ...
            window.Store.AppState.on('change:state', (_AppState, state) => { 
                console.log('🔥 DEBUG: Store.AppState change:state event triggered, state:', state);
                window.onAppStateChangedEvent(state); 
            });
            
            window.Store.Conn.on('change:battery', (state) => { 
                console.log('🔥 DEBUG: Store.Conn change:battery event triggered');
                window.onBatteryStateChangedEvent(state); 
            });
            
            window.Store.Call.on('add', (call) => { 
                console.log('🔥 DEBUG: Store.Call add event triggered');
                window.onIncomingCall(call); 
            });
            
            window.Store.Chat.on('remove', async (chat) => { 
                console.log('🔥 DEBUG: Store.Chat remove event triggered for chat:', chat.id?._serialized);
            ...
            window.Store.Chat.on('change:archive', async (chat, currState, prevState) => { 
                console.log('🔥 DEBUG: Store.Chat change:archive event triggered for chat:', chat.id?._serialized);
                window.onArchiveChatEvent(await window.WWebJS.getChatModel(chat), currState, prevState); 
            });
            
            // MOST IMPORTANT: Message add event
            window.Store.Msg.on('add', (msg) => { 
                console.log('🔥 DEBUG: Store.Msg ADD event triggered!', {
                    messageId: msg.id?._serialized,
                ...
                if (msg.isNewMsg) {
                    if(msg.type === 'ciphertext') {
                        console.log('🔥 DEBUG: Message is ciphertext, deferring until type changes');
                        // defer message event until ciphertext is resolved (type changed)
                        msg.once('change:type', (_msg) => {
                            console.log('🔥 DEBUG: Ciphertext resolved, triggering onAddMessageEvent');
                            window.onAddMessageEvent(window.WWebJS.getMessageModel(_msg));
                        });
                        window.onAddMessageCiphertextEvent(window.WWebJS.getMessageModel(msg));
                    } else {
                        console.log('🔥 DEBUG: Triggering onAddMessageEvent directly');
                        window.onAddMessageEvent(window.WWebJS.getMessageModel(msg)); 
                    }
                } else {
                    console.log('🔥 DEBUG: Message is not new, skipping');
                }
            });
            
            window.Store.Chat.on('change:unreadCount', (chat) => {
                console.log('🔥 DEBUG: Store.Chat change:unreadCount event triggered for chat:', chat.id?._serialized);
                window.onChatUnreadCountEvent(chat);
            });
            
            console.log('✅ DEBUG: All WhatsApp Store event listeners have been set up');
