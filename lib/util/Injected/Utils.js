...
exports.LoadUtils = () => {
    ...
    window.WWebJS.sendClearChat = async (chatId) => {
        throw new Error('Read-only mode: clearMessages is disabled');
    ...
    window.WWebJS.sendDeleteChat = async (chatId) => {
        throw new Error('Read-only mode: deleteChat is disabled');
        let chat = await window.WWebJS.getChat(chatId, { getAsModel: false });
        if (chat !== undefined) {
    ...
    window.WWebJS.sendChatstate = async (state, chatId) => {
        throw new Error('Read-only mode: chat state is disabled');
    ...
    window.WWebJS.subscribeToUnsubscribeFromChannel = async (channelId, action, options = {}) => {
        throw new Error('Read-only mode: channel subscription is disabled');
    ...
    window.WWebJS.pinUnpinMsgAction = async (msgId, action, duration) => {
        throw new Error('Read-only mode: pin/unpin message is disabled');
